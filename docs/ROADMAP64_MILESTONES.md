...
| M5 | Sound (audio backend) | Parity with 32-bit audio: wine's audio stack → an SDL-audio (WebAudio) device in the browser | DOOM plays its sound effects (or a .wav plays via sndrec/winmm test) audibly in the tab; AudioContext confirmed feeding samples | WORKING (2026-07-04, browser-verified headlessly): the winmm test app plays PCM to completion through the full chain — winmm → winealsa → libasound (plug) → NEW pcm_oss plugin + ALSA config in the rootfs → /dev/dsp via a NEW 64-bit OSS ioctl path (FsOpenNode::ioctl64) → KDspAudio → SDL/WebAudio. Three fixes: rootfs had NO /usr/share/alsa config (libasound couldn't resolve any PCM name), sys_ioctl64 ENOTTY'd all device ioctls, and an obsolete 11 kHz emscripten frequency cap made stream creation fail. Remaining for full DONE: audible spot-check by a human + rebuild DOOM with real i_sound (it ships the dummy) |
...
| M7 | Lazy / streamable rootfs | Don't download ~196MB before first paint: HTTP-Range-backed zip reads (or progressive mount), unlocks bigger bundled apps (Quake 2 deferral) | First app reaches first paint with materially less than the full rootfs downloaded (measure bytes-before-first-paint before/after) | WORKING (2026-07-04, opt-in `?lazy=1`, browser-verified): wine64.zip mounts FROM ITS URL via a minizip Range filefunc (source/io/fszipurl.cpp — 512KB blocks, sync XHR on the guest pthread workers, session block cache; handles both the whole zip and the Pages split parts via the manifest). Measured: notepad first paint at 33s with ~83MB total downloaded (28MB eager glibc+prefix + ~55MB lazy) vs 234MB before = **65% less**, and boot got slightly FASTER; glcube and d3dtri boot and RENDER lazily too. server.mjs gained HTTP Range support for local testing. Default stays eager pending a live soak; flip `lazy` default after |
