...
| M18 | Boot reliability | Kill the two known boot flakes so a cold boot succeeds ~always: (a) the wineserver startup race (several wineservers contend, the real one loses and exits → app never paints — observed ~1-in-3 headless boots since 2026-06-11), (b) the d3dtri/GL `CreateDevice` early-exit flake (~1-in-2, app exits status=1 ~80s in before GL init). Root-cause each, fix in the spawn/serialization path (not by retry) | 10 consecutive cold headless boots of notepad AND glcube AND d3dtri all paint without a retry, locally; CI smoke passes on attempt 1 for 3 consecutive deploys | IN PROGRESS (2026-07-04) — TRUE root cause found via a 10-boot repro harness (/Users/nakas/winebuild/boot_loop.mjs): wineserver closes an "empty" desktop after 1 SECOND (server/winstation.c `close_desktop_timeout`), and under the interpreter the wineboot→app handoff leaves the desktop empty for many seconds → explorer /desktop gets WM_CLOSE mid-boot (observed exiting cleanly ~51s in every failing log) → the late app finds no desktop. Fix = patched wineserver (desktop grace 600s, winebuild Docker) + kernel-side `-p` append on bare wineserver spawns (sys_execve64) so the session server is always persistent (the old `-p` pin only won its lock race ~half the time). Validation loop pending |
